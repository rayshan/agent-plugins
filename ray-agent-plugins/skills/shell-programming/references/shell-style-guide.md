...
* **Invocation:** Last line of script must invoke `main "$@"`, either bare or wrapped in a source guard for testability: `if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then main "$@"; fi`.
* **Variables:**
...
* **Loops:** Prefer `while IFS= read -r ... < <(cmd)` over `cmd | while` to avoid subshell variable loss.
...
* **Function Comments:** Required for all non-trivial/library functions. Format:

```bash
# Processes a configuration file and outputs parsed values.
#
# Globals:
#   CONFIG_DIR
# Arguments:
...
# Outputs:
#   Writes parsed values to stdout
# Returns:
#   0 on success, 1 on invalid format, 2 on missing file
parse_config() {
```
